...
def create_account_key(model_uuid, application_name, relation_id):
    ...
    with TemporaryDirectory() as tempdir:
        ...
        log(
            "Created service account key {} for service account {}",
            key_id,
            service_account,
        )
        creds = tempfile.read_text()
    kv().set(
        "charm.gcp.account-keys.{}".format(relation_id),
        {
...
def label_instance(instance, zone, labels):
    ...
    _gcloud(
        ...
        zone,
        "--labels",
        ",".join("=".join([k, v]) for k, v in labels.items()),
    )
...
def enable_block_storage_management(model_uuid, application_name):
    ...
    service_account = _get_service_account(model_uuid, application_name)
    _ensure_custom_role(
        name="compute.instanceStorageAdmin",
        title="Storage admin for instances",
        description="Attach and remove disks to instances",
        permissions=["compute.instances.attachDisk", "compute.instances.detachDisk"],
    )
    _add_roles(
        service_account,
        [
...
def cleanup(relation_ids):
    ...
    for relation_id in broken:
        ...
        log(
            "Deleted unused key {} for service account {}",
            key["id"],
            key["service-account"],
        )
...
def _get_service_account(model_uuid, application_name):
    ...
    service_accounts.update(
        {sa["email"].split("@")[0]: sa["email"] for sa in cloud_service_accounts}
    )
    ...
    log("Created service account for {}: {}", application_name, service_account)
    kv().set(sa_cache_key, service_accounts)
